...
const TeamActivity = ({ teamMembers, tasks }) => {
  ...
  const activeMembersOnly = allMembersWithStats.filter(member => member.stats.total > 0);
  ...
  const shouldShowToggle = teamMembers.length > 5 || activeMembersOnly.length > 5;
  ...
  if (!teamMembers || teamMembers.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-gray-500">Add team members to see activity</p>
      </div>
    );
  }
  ...
  const renderMember = (member, index) => (
    <div key={member.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
      <div className="relative">
        ...
        {index === 0 && member.stats.completionRate >= 90 && member.stats.total > 0 && (
      ...
      <div className="flex-1 min-w-0">
        ...
        {member.stats.total > 0 ? (
          ...
          <div className="mt-1">
            <span className="text-xs text-gray-500 italic">No tasks assigned</span>
          </div>
        )}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-600">
          {activeMembersOnly.length} active of {teamMembers.length} total members
        </div>
        {shouldShowToggle && (
          <button
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              <div className="text-lg font-semibold text-green-600">
                {activeMembersOnly.filter(member => member.stats.completionRate >= 80).length}
              </div>
              <div className="text-xs text-gray-600">High Performers</div>
            </div>
            <div>
              <div className="text-lg font-semibold text-blue-600">
                {Math.round(activeMembersOnly.reduce((acc, member) => acc + member.stats.completionRate, 0) / activeMembersOnly.length) || 0}%
              </div>
